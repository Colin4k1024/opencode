...
import {
  ...
  useCommand,
  useWslServers,
} from "@cangming-ai/app"
import type { UpdaterState } from "@cangming-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@cangming-ai/ui/logo"
import { useTheme } from "@cangming-ai/ui/theme/context"
...
function windowLastActiveUrlKey(windowID: string) {
  return `cangming.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://cangming.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("cangming.global.dat").getItem("language")
