...
function Orchestrate(props: ToolProps<typeof OrchestrateTool>) {
  const { theme } = useTheme()
  const { navigate } = useRoute()
  ...
  const totalDuration = metadata.totalDuration as number | undefined
  const currentOutput = metadata.currentOutput as string | undefined

  const [expandedOutput, setExpandedOutput] = createSignal(false)
  const [expandedSteps, setExpandedSteps] = createSignal(false)

  // Extended phase labels for all 7 phases
  const phaseLabels: Record<string, string> = {
    analyze: "🔍 Analyze TODO",
    plan: "📋 Plan",
    build: "🔨 Build",
    test_plan: "📝 Plan Tests",
    test_run: "🧪 Run Tests",
    test: "🧪 Test",
    fix: "🔧 Fix",
    commit: "💾 Commit",
  ...
  // Phase definitions for the 7 phases
  const phaseDefinitions = [
    { id: "analyze", label: "🔍 Analyze TODO", prefix: "analyze_" },
    { id: "plan", label: "📋 Plan", prefix: "plan_" },
    { id: "build", label: "🔨 Build", prefix: "build_" },
    { id: "test_plan", label: "📝 Plan Tests", prefix: "test_plan_" },
    { id: "test_run", label: "🧪 Run Tests", prefix: "test_run_" },
    { id: "fix", label: "🔧 Fix", prefix: "fix_" },
    { id: "commit", label: "💾 Commit", prefix: "commit_" },
  ]

  // Identify phase from step ID
  const getPhaseFromStepId = (stepId: string): string | undefined => {
    const stepIdLower = stepId.toLowerCase()
    for (const phaseDef of phaseDefinitions) {
      if (stepIdLower.startsWith(phaseDef.prefix)) {
        return phaseDef.id
      }
    }
    return undefined
  ...
  const stepsByPhase = createMemo(() => {
    ...
    for (const step of steps) {
      ...
      grouped[phaseId].push(step)
    }
    return grouped
  })

  // Calculate phase completion status
  const getPhaseStatus = (phaseId: string): { completed: boolean; hasSteps: boolean; allSuccess: boolean } => {
    const phaseSteps = stepsByPhase()[phaseId] || []
    if (phaseSteps.length === 0) {
      return { completed: false, hasSteps: false, allSuccess: false }
    }
    const allSuccess = phaseSteps.every((s) => s.success || s.skipped)
    return { completed: true, hasSteps: true, allSuccess }
  }
  ...
  // Check if output should be expandable
  const outputLines = createMemo(() => {
    const output = props.output || currentOutput || ""
    return output.split("\n")
  })
  const outputOverflow = createMemo(() => outputLines().length > 20)
  const limitedOutput = createMemo(() => {
    if (expandedOutput() || !outputOverflow()) return outputLines().join("\n")
  ...
  return (
    <BlockTool
      ...
      part={props.part}
      onClick={
        isCompleted() && (outputOverflow() || (steps && steps.length > 0))
          ? () => {
              if (outputOverflow()) {
    ...
    >
      <box gap={1}>
        {/* Workflow Overview - Show all 7 phases */}
        <Show when={isCompleted()}>
          <box flexDirection="column" gap={0}>
            <text fg={theme.text}>
              <span style={{ bold: true }}>Workflow Progress</span>
            </text>
        ...
        >
          <box flexDirection="column" gap={0}>
            <text fg={theme.textMuted}>
              ...
              {totalSteps !== undefined ? `${stepIndex || totalSteps}/${totalSteps}` : steps?.length || 0}
              <Show when={successCount !== undefined}>
                {" "}
                | <span style={{ fg: theme.success }}>✓ {successCount}</span>
              </Show>
              <Show when={failedCount !== undefined && failedCount > 0}>
                {" "}
                | <span style={{ fg: theme.error }}>✗ {failedCount}</span>
              </Show>
              <Show when={skippedCount !== undefined && skippedCount > 0}>
                {" "}
        ...
        {/* Current Step Info (when executing) */}
        <Show when={!isCompleted()}>
          <box flexDirection="column" gap={0}>
            {/* Workflow Progress Header */}
            <Show when={workflow}>
              <text fg={theme.text}>
                <span style={{ bold: true }}>Workflow: {workflow}</span>
              </text>
            </Show>
            ...
            {/* Step Progress */}
            <Show when={stepIndex !== undefined && totalSteps !== undefined}>
              <text style={{ fg: theme.text }}>
                Step {stepIndex}/{totalSteps}
                <Show when={metadata.currentStep}>
                  {" "}
                  <span style={{ fg: theme.textMuted }}>({metadata.currentStep})</span>
                </Show>
              </text>
            </Show>
            ...
            {/* Agent and Status */}
            <Show when={agent}>
              <text style={{ fg: theme.textMuted }}>Agent: @{agent}</text>
            </Show>
            <Show when={status}>
              <text
                style={{
                  fg: status === "completed" ? theme.success : status === "failed" ? theme.error : theme.textMuted,
                }}
              >
                Status: {status}
              </text>
            ...
            <Show when={progress !== undefined}>
              <text style={{ fg: theme.textMuted }}>Progress: {progress}%</text>
            </Show>
          </box>
        </Show>
          ...
          <box flexDirection="column" gap={0}>
            ...
            <Show when={expandedSteps()}>
              <box flexDirection="column" gap={0} paddingLeft={2}>
                <For each={steps}>
                  {(step) => {
                    const stepPhase = getPhaseFromStepId(step.id)
                    const statusIcon = step.skipped ? "⏭" : step.success ? "✓" : "✗"
                    const statusColor = step.skipped ? theme.textMuted : step.success ? theme.success : theme.error
                    const stepPhaseLabel = stepPhase ? phaseLabels[stepPhase] : undefined
                    ...
                    return (
                      ...
                      >
                        <span style={{ fg: statusColor }}>{statusIcon}</span> {step.id}
                        <Show when={step.agent}>
                          {" "}
                          <span style={{ fg: theme.textMuted }}>(@{step.agent})</span>
                        </Show>
                        <Show when={stepPhaseLabel && !step.agent}>
                          {" "}
                          <span style={{ fg: theme.textMuted }}>({stepPhaseLabel})</span>
                        </Show>
                        <Show when={step.sessionID}>
                          {" "}
                          <span style={{ fg: theme.textMuted }}>
                            <span style={{ textDecoration: "underline" }}>→ session</span>
                          </span>
                        </Show>
                      </text>
                    )
            ...
            <Show when={expandedOutput() || !outputOverflow()}>
              <box paddingLeft={2}>
                ...
                <Show when={outputOverflow()}>
                  <text fg={theme.textMuted}>{expandedOutput() ? "Click to collapse" : "Click to expand"}</text>
                </Show>
              </box>
            </Show>
          </box>
