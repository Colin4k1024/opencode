...
function detectNaturalLanguageProposal(text: string): FixProposal | null {
  ...
  if ((problemMatch || rootCauseMatch) && (needsConfirmation || riskLevel !== "low")) {
    ...
    while ((optionMatch = optionPattern.exec(text)) !== null) {
      ...
      const solutionDesc = solutionDescMatch ? solutionDescMatch[1].trim() : optionBlock.split(/\n/)[0].trim()

      // 提取风险级别（从 Option 块中）
      const optionRiskMatch = optionBlock.match(/[-*]\s*Risk Level[:\s]*(low|medium|high)/i)
      const optionRisk = optionRiskMatch ? (optionRiskMatch[1].toLowerCase() as "low" | "medium" | "high") : riskLevel
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: OpencodeClient, sessionID: string) => {
      ...
      const eventProcessor = (async () => {
        ...
        timeoutCheckInterval = setInterval(async () => {
          ...
          if (
            timeSinceLastUpdate > ANALYSIS_TIMEOUT &&
            !proposalDetected &&
            timeSinceLastReminder > REMINDER_COOLDOWN
          ) {
              ...
              if (args.format !== "json" && process.stdout.isTTY && !args.auto) {
                ...
                if (proposal && !pendingProposal) {
                  ...
                  if (!processedProposals.has(proposalId)) {
                    ...
                    const isMediumRisk =
                      proposal.fix_proposals.some((fp) => fp.risk_level === "medium") &&
      ...
      prompt +=
        "1. Use glob tools to scan the project root directory to identify the main tech stack and project structure\n"
      prompt +=
        "2. Use config-reader tool to automatically load core configuration files (package.json, tsconfig.json, .eslintrc, etc.)\n"
      ...
      prompt +=
        "5. Report initialization completion status, including identified tech stack, project type, and available tools\n\n"
      ...
      if (args.auto) {
        ...
        prompt +=
          "After completing your analysis, you MUST describe the problem, root cause, and solution in natural language (NOT JSON format).\n"
        prompt +=
          "For medium-risk issues with multiple solutions, you MUST autonomously select the best one and explain your choice.\n"
        prompt +=
          "Do NOT continue analyzing indefinitely. Once you have identified the most likely root cause, propose a solution immediately.\n"
        ...
        prompt +=
          "   - **For database-related errors**: First identify project type. For Spring Boot, read application.yml/application.properties from src/main/resources/. Extract database connection info and use sql tool to query the database\n"
        ...
        prompt +=
          "   - For LOW-RISK issues: Describe problem, root cause, and solution, then IMMEDIATELY execute the fix using edit/write/bash tools\n"
        prompt +=
        ...
        prompt +=
          '   - For HIGH-RISK issues: Describe problem, root cause, and proposed solutions with pros/cons. Explicitly state "需要用户确认" (This fix requires user confirmation), then wait for confirmation before executing\n'
        prompt +=
        ...
        prompt += "A fix proposal that may need refinement is better than no proposal at all.\n"
        prompt +=
          "NEVER terminate your response after just describing a fix - you MUST continue to execute it (for low/medium-risk) or wait for confirmation (for high-risk).\n"
      }
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sessionID = await (async () => {
        const title =
          args.title ||
          (errorMessage ? errorMessage.slice(0, 50) + (errorMessage.length > 50 ? "..." : "") : "Fix Error")
